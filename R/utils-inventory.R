...
#' @return data.frame
#' @export
clean_invetory_cpe <- function(df_inventory = data.frame(), df_ner = data.frame()) {
  df_chk <- dplyr::left_join(df_inventory, df_ner %>% select(-"title"), by = "id")
  ...
  df_final <- dplyr::arrange(df_final, .data$id)
